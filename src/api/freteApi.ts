...
export async function buscarFrete(clienteId: number, cidade: string, valorNota?: number): Promise<FreteCliente | null> {
  const params = new URLSearchParams()
  params.append("clienteId", String(clienteId))
  params.append("cidade", cidade)
  if (valorNota !== undefined) params.append("valorNota", String(valorNota))
...
export async function salvarFrete(dto: { clienteId: number, cidade?: string, valor?: number, tipo?: TipoFrete, percentual?: number, adicional?: number }): Promise<FreteCliente> {
