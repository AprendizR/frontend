...
export function DashboardPage() {
  ...
  <main className="px-6 py-8 space-y-8">
    ...
    <section>
      ...
      {loading ? (
        <div className="text-center py-12 text-slate-500">Carregando estatísticas...</div>
      ) : stats ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <StatCard title="Total de Cargas" value={stats.totalCargas} icon="🚛" color="blue" />
          <StatCard title="Em Rota" value={stats.cargasEmRota} icon="🛣️" color="orange" />
          <StatCard title="Cargas Entregues" value={stats.cargasEntregues} icon="✅" color="green" />
          <StatCard title="Total de Notas" value={stats.totalNotas} icon="📄" color="purple" />
          <StatCard title="Notas Pendentes" value={stats.notasPendentes} icon="⏳" color="red" />
          <StatCard title="Entregues Hoje" value={stats.notasEntreguesHoje} icon="📦" color="green" subtitle="Desde 00:00 de hoje" />
        </div>
      ) : (
